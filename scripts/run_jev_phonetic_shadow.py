...
def prefix_beam(
    log_probs: Any, blank: int, width: int = 8, frame_top_k: int | None = 5
    ...
    for frame in log_probs:
        ...
        def add(prefix: tuple[int, ...], channel: int, score: float, target=following) -> None:
            pair = target.setdefault(prefix, [-math.inf, -math.inf])
    ...
    return sorted(
        ((key, logadd(*pair)) for key, pair in beam.items()), key=lambda pair: (-pair[1], pair[0])
    )
...
def validate_record(record: dict[str, Any]) -> None:
    ...
    if (
        next(c["text"] for c in candidates if c["id"] == record["baseline_id"])
...
def build_request(
    record: dict[str, Any], alternatives: list[list[str]], arm: str
    ...
    hypotheses = [
        {"id": alias, "phones": list(candidate["phone_symbols"])}
    ...
    state = {
        ...
        "symbol_legend": (
            "Japanese phone labels; N=moraic nasal, cl=closure/gemination; "
            "repeated vowels can represent length. Labels are not words."
        ),
    }
    criteria = {
        alias: "This candidate is uniquely supported by the observed phonemes." for alias in aliases
    }
...
class Client:
    ...
    def __init__(self, key: str, max_calls: int, max_input_tokens: int, max_wall_seconds: int):
        ...
        if (
            not 1 <= max_calls <= 600
            or not 64000 <= max_input_tokens <= 4000000
            or not 1 <= max_wall_seconds <= 3600
        ):
    ...
    def call(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            ...
            or self.input_tokens + 64000 > self.max_tokens
        ...
        try:
            with self.opener.open(
                req, timeout=min(20, max(1, self.deadline - started))
            ) as response:
...
def selected_or_baseline(
    record: dict[str, Any], choice: str, aliases: dict[str, str], observation_available: bool = True
) -> tuple[str, str | None]:
    ...
    identical = [
        c for c in record["candidates"] if c["phone_symbols"] == candidate["phone_symbols"]
    ]
    if len(identical) > 1:
...
def acoustic_features(
    record: dict[str, Any], probe: Path, manifest: dict[str, Any], vocab: dict[str, int]
    ...
    alternatives = [[inverse[i] for i in prefix] for prefix, _ in prefix_beam(logs, blank)[:3]]
    frames = tuple(
    ...
    posterior = PosteriorSequence(
        kind="phone",
        blank_symbol=inverse[blank],
        vocabulary=tuple(inverse[i] for i in range(len(vocab))),
        frames=frames,
        encoder=manifest["phone_model"],
        encoder_revision=manifest["phone_revision"],
        label_set_revision=file_digest(probe / "vocab.json"),
        source_audio_sha256=record["pcm_sha256"],
    )
...
def summarize(rows: list[dict[str, Any]], receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    diagnostics = {
        ...
        "raw_reorder_disagreements": 0,
        "reorder_pairs": 0,
        "raw_repeat_disagreements": 0,
        "repeat_pairs": 0,
    ...
    for row in rows:
        for result in row["arms"].values():
            diagnostics["homophone_gate_violations"] += (
                result.get("gate_reason") == "indistinguishable-pronunciations"
            )
        ...
        for arm, name in (("reordered", "reorder"), ("repeat", "repeat")):
            ...
            if a.get("raw_choice") is not None and b.get("raw_choice") is not None:
                ...
                diagnostics["raw_" + name + "_disagreements"] += a.get("raw_selected_id") != b.get(
                    "raw_selected_id"
                )
    return {
        "receipt": receipt,
        "diagnostics": diagnostics,
        "normalization": "NFKC; remove Unicode punctuation and whitespace; case preserved",
        "arms": dict(totals),
        "promotion": "not-evaluated",
        "new_weights_trained": False,
        "warning": (
            "Exposed-development, one corpus, no verified speaker separation "
            "or gold phone annotations."
        ),
    }
...
def run(args: argparse.Namespace, key: str | None = None) -> dict[str, Any]:
    ...
    if (
        manifest.get("dataset") != "google/fleurs"
        or manifest.get("subset") != "ja_jp"
    ...
    if not manifest.get("complete") and not args.allow_partial:
        raise ValueError(
            "collection is not complete; use explicit --allow-partial for a dated snapshot"
        )
    ...
    client = (
        Client(
            key or os.environ.get("TYPESAFE_API_KEY", ""),
            args.max_calls,
    ...
    try:
        for index, entry in enumerate(entries):
            ...
            errors = {
                c["id"]: edits(reference, normalized(c["text"])) for c in record["candidates"]
            }
            baseline = record["baseline_id"]
            row = {
                "id": sample,
                ...
                "oracle_exact_in_candidates": min(errors.values()) == 0,
                "features": features,
                "arms": {
                    "baseline": {"selected_id": baseline, "errors": errors[baseline]},
            ...
            if client and any(
                result.get("response", {}).get("status") != "received"
                or result.get("gate_reason") == "malformed-response"
                for row in rows
                for arm, result in row["arms"].items()
                if arm not in {"baseline", "local_ctc", "oracle"}
            ):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    for name, lower, upper in [
        ("max_records", 1, 96),
        ("max_calls", 1, 600),
        ("max_input_tokens", 64000, 4000000),
