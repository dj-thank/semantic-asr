    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, raw in enumerate(handle, 1):
            ...
            if not group_id or not isinstance(candidate_rows, list):
                raise ValueError(
                    f"{path}:{line_number}: groupId and candidates are required"
                )
            ...
            for candidate_index, candidate_row in enumerate(candidate_rows):
                ...
                candidate_id = str(
                    candidate_row.get("candidateId")
                    or candidate_row.get("candidate_id")
                    or ""
                )
                raw_features = candidate_row.get("features")
                if not candidate_id or not isinstance(raw_features, dict):
                    raise ValueError(
                        f"{path}:{line_number}: candidate ID/features are required"
                    )
                values = {
...
def _load_schema(path: Path | None) -> FeatureSchema:
    ...
    monotonicity = {
        str(name): int(value)
        for name, value in dict(row.get("monotonicity") or {}).items()
    }
