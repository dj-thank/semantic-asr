...
def main():
    ...
    excluded_ids = set(prior.get("prior_excluded_source_ids", ()))
    excluded_audio = set(prior.get("prior_excluded_audio_sha256", ()))
    excluded_text = set(prior.get("prior_excluded_reference_sha256", ()))
    dev_rows = {}
    for item in prior["records"]:
        ...
        excluded_text.add(hashlib.sha256(row["reference"].encode()).hexdigest())
    ...
    manifest.update(
        {
            ...
            "selection": (
                "prior development audio; fresh test excludes all prior text IDs/audio/text"
            ),
            "prior_excluded_source_ids": sorted(excluded_ids),
            "prior_excluded_audio_sha256": sorted(excluded_audio),
            "prior_excluded_reference_sha256": sorted(excluded_text),
            "candidate_expansion": {
                "baseline_beam": 5,
                "extra_beam": 12,
                "extra_hypotheses": 12,
                "same_model_second_decode": True,
                "reference_prompted": False,
            },
    ...
    for split in ("validation", "test"):
        count = 0
        parquets = sorted(
            f for f in files if "ja_jp/" in f and f"/{split}" in f and f.endswith(".parquet")
        )
        for name in parquets:
            file = hf_hub_download(
                prior["dataset"], name, repo_type="dataset", revision=prior["dataset_revision"]
            )
            for batch in pq.ParquetFile(file).iter_batches(batch_size=8):
                for raw in batch.to_pylist():
                    ...
                    if split == "test" and (
                        uid in excluded_ids
                        or hashlib.sha256(ref.encode()).hexdigest() in excluded_text
                    ):
                        continue
                    ...
                    try:
                        ...
                        begun = time.perf_counter()
                        extra = warm.base_adapter.decode(
                            DecodeRequest(
                                str(path),
                                beam_size=12,
                                hypotheses=12,
                    ...
                    for candidate in extra:
                        if candidate.text not in texts:
                            ...
                            candidates.append(
                                replace(
                                    candidate,
                                    candidate_id="expanded:" + candidate.candidate_id,
                                    metadata={
                                        **candidate.metadata,
                                        "decodeNamespace": "wave2-beam12",
                                        "decodeStartMs": 0,
                                        "decodeEndMs": result.duration_ms,
                                    },
                                )
                            )
                    rows = [
                        {
                            "id": c.candidate_id,
                            "text": c.text,
                            "phone_symbols": pyopenjtalk.g2p(c.text).split(),
                            "kana": pyopenjtalk.g2p(c.text, kana=True),
                            "avg_logprob": c.avg_logprob,
                            "sequence_score": c.sequence_score,
                            "acoustic": c.acoustic,
                            "rank": c.rank,
                            "evidence": c.as_dict(),
                        }
                    ...
                    record = {
                        ...
                        "reference": ref,
                        "reference_phones": pyopenjtalk.g2p(ref).split(),
                        "reference_kana": pyopenjtalk.g2p(ref, kana=True),
                        ...
                        "asr_seconds": base_seconds,
                        "expansion_seconds": extra_seconds,
                        "posterior_sha256": digest(out / f"{sid}.npz"),
                    }
                    ...
                    manifest["records"].append(
                        {
                            ...
                            "split": split,
                            "pcm_sha256": pcm_sha,
                            "record_sha256": digest(out / f"{sid}.json"),
                        }
                    )
                    write_json(out / "manifest.json", manifest)
                    print(
