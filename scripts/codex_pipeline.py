...
PACKAGES = ("semantic-asr", "pytest", "ruff", "build", "setuptools", "wheel", "numpy",
            "torch", "safetensors", "soundfile", "faster-whisper", "ctranslate2")
...
def environment() -> dict[str, Any]:
    ...
    return {"python": platform.python_version(), "platform": platform.system(),
            "machine": platform.machine(), "packages": versions, "is_dependency_lock": False}
...
def run_stage(name: str, command: list[str], output: Path, receipt: dict[str, Any],
              deadline: float, storage: int, *, cwd: Path = ROOT) -> None:
...
def junit_counts(path: Path) -> dict[str, int]:
    ...
    return {"tests": len(cases), "failures": sum(x.find("failure") is not None for x in cases),
...
def check(args: argparse.Namespace, output: Path, receipt: dict[str, Any], stage: Any) -> None:
    ...
    try:
        stage("pytest", [sys.executable, "-m", "pytest", "-q", f"--junitxml={output / 'tests.xml'}"])
    finally:
        if (output / "tests.xml").exists():
            receipt["tests"] = junit_counts(output / "tests.xml")
    if args.lane == "training-cpu" and receipt["tests"]["skipped"]:
        raise ValueError("training-cpu lane has skipped tests; inspect JUnit instead of claiming coverage")
    ...
    stage("model-free-optimization", [sys.executable, "scripts/run_v02_model_free_validation.py",
                                      "--output", str(output / "optimization.json")])
    ...
    stage("wheel-install", [str(python), "-m", "pip", "install", "--no-deps", "--no-index",
                            str(wheels[0])], cwd=output)
    code = ("import pathlib, semantic_asr; "
            f"assert pathlib.Path(semantic_asr.__file__).resolve().is_relative_to(pathlib.Path({str(venv)!r})); "
            "[getattr(semantic_asr, n) for n in semantic_asr.__all__]; "
            "from semantic_asr.cli_root import main; raise SystemExit(main())")
    for command, extra in (("demo", ["--output", str(output / "wheel-demo.json")]),
...
def research_input(args: argparse.Namespace) -> dict[str, Any]:
    if not args.allow_local_research:
        raise Blocked("explicit --allow-local-research is required; it does not authorize publication")
    ...
    return {"manifest_sha256": sha256(manifest), "audio_sha256": identities,
            "record_count": len(records), "audio_seconds": seconds, "model": model,
            "split_counts": {split: sum(row.split == split for row in records)
                             for split in ("train", "calibration", "test")},
            "evaluation_role": args.evaluation_role, "speaker_disjointness_verified": False}
...
def research(args: argparse.Namespace, output: Path, receipt: dict[str, Any], stage: Any) -> None:
    ...
    executable = Path(sysconfig.get_path("scripts")) / ("semantic-asr.exe" if os.name == "nt" else "semantic-asr")
    if not executable.is_file():
        raise Blocked("install semantic-asr in the active Python environment")
    model_flags = (["--model-artifact-sha256", receipt["inputs"]["model"]["artifact_sha256"]]
                   if "artifact_sha256" in receipt["inputs"]["model"] else
                   ["--model-revision", receipt["inputs"]["model"]["revision"]])
    ...
    stage("generate", [*CLI, "generate-candidates", str(Path(args.manifest).expanduser().resolve()),
                       "--output", str(candidates), "--allow-raw-export", "--model", args.model,
                       *model_flags, "--runtime-revision", runtime, "--device", args.device,
                       "--compute-type", args.compute_type, "--cpu-threads", "2",
                       "--beam-size", str(args.beam_size), "--hypotheses", str(args.hypotheses)])
    stage("post-candidate", [sys.executable, "scripts/run_real_audio_pipeline.py", "--candidates",
                             glob.escape(str(candidates)), "--output-dir", str(output / "pipeline"),
                             "--allow-raw-export", "--ranker", args.ranker, "--cli", str(executable),
    ...
    for name in ("report.json", "report-raw.json"):
        ...
        if (type(report.get("sample_count")) is not int
...
def main(argv: list[str] | None = None) -> int:
    ...
    commands.add_parser("plan", help="print implemented stages without running/installing/downloading")
    validation = commands.add_parser("check", help="validate code without repairing or publishing it")
    ...
    experiment.add_argument("--evaluation-role", choices=("development", "regression-exposed"), required=True)
    ...
    experiment.add_argument("--compute-type", choices=("int8", "float16", "int8_float16", "float32"), default="int8")
    ...
    if args.command == "plan":
        print(json.dumps({"check": ["format", "lint", "compile", "pytest", "regression-fixtures",
                                    "synthetic-optimization", "CLI-discovery", "outside-checkout-wheel"],
                          "research": ["authorize-and-bound", "generate", "existing-post-candidate-driver"],
                          "trials": 1, "automatic_promotion": False,
                          "handoff": "docs/development/CODEX_HANDOFF.md"}, indent=2))
        return 0
    try:
    ...
    receipt: dict[str, Any] = {"schema": "codex-pipeline-receipt-v1", "command": args.command,
                              "status": "running", "stages": [], "environment": environment(),
                              "max_wall_seconds": args.max_wall_seconds,
                              "max_storage_bytes": args.max_storage_bytes, "trials": 1,
                              "promotion": "not-evaluated", "automatic_publish": False}
    ...
    try:
        ...
        # Local receipt may contain private paths. Never upload research receipts/logs automatically.
        ...
        try:
            ...
            if receipt.get("source") != receipt["source_after"]:
                ...
                code = 1
            receipt["artifacts"] = {str(path.relative_to(output)): {"sha256": sha256(path),
