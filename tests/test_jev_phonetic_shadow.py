...
@pytest.fixture
def record():
    return {'id':'validation-000', 'baseline_id':'orig-a', 'baseline_text':'食べれる',
            'phone_greedy':['t','a','b','e','r','e','r','u'],
            'reference':'NEVER_SEND_REFERENCE', 'reference_phones':['GOLD_SENTINEL'],
            'reference_kana':'NEVER_SEND_KANA', 'context':'NEVER_SEND_CONTEXT',
            'candidates':[
                {'id':'orig-a','text':'食べれる','phone_symbols':['t','a','b','e','r','e','r','u'],
                 'sequence_score':987654321, 'rank':1},
                {'id':'orig-b','text':'食べられる','phone_symbols':['t','a','b','e','r','a','r','e','r','u'],
                 'sequence_score':123456789, 'rank':2}]}


def answer(choice='c00'):
    return {'model':m.MODEL, 'answers':{'supported_utterance':{
...
def test_strict_payload_projection(record, arm):
    ...
    for forbidden in ['NEVER_SEND','GOLD_SENTINEL','食べれる','食べられる',
                      '987654321','123456789','sequence_score','orig-a','reference','context']:
...
@pytest.mark.parametrize('choice,reason', [('abstain','model-abstained'),
                                          ('outside','invalid-id')])
...
def test_missing_audio_gate(record):
    assert m.selected_or_baseline(record, 'c01', {'c01':'orig-b'}, False) == ('orig-a','missing-observation')


def test_homophone_gate(record):
    record['candidates'][1]['phone_symbols'] = list(record['candidates'][0]['phone_symbols'])
    assert m.selected_or_baseline(record, 'c01', {'c01':'orig-b'}) == ('orig-a','indistinguishable-pronunciations')
...
def test_unnormalized_probability():
    value = answer()
    value['answers']['supported_utterance']['probabilities']['abstain'] = .7
    with pytest.raises(ValueError):
        m.validate_answer(value, {'c00':'a','c01':'b'})
...
def collapse(path, blank=0):
    return tuple(symbol for i, symbol in enumerate(path)
...
def test_ctc_prefix_matches_exhaustive_path_sum(frames):
    probabilities = [[.2,.5,.3], [.3,.2,.5], [.6,.3,.1], [.1,.4,.5]][:frames]
...
def test_repeated_phone_requires_blank_for_two_outputs():
    logs = [[math.log(x) for x in row] for row in [[.01,.99],[.99,.01],[.01,.99]]]
    assert m.prefix_beam(logs,0,20,None)[0][0] == (1,1)


def test_pruned_ctc_only_has_valid_sequences():
    logs = [[math.log(x) for x in row] for row in [[.2,.5,.3]]*3]
...
@pytest.mark.parametrize('field,value',[('--max-records','97'),('--max-calls','601'),
                                        ('--max-input-tokens','4000001'),('--max-wall-seconds','3601')])
