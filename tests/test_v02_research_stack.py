...
def test_candidate_pool_preserves_duplicate_path_mass() -> None:
    pool = _pool()
    tokyo_ni = next(candidate for candidate in pool.candidates if candidate.text == "今日は東京に行く")
...
def test_mbr_selects_only_existing_candidate() -> None:
    pool = _pool()
    posterior = {
        next(candidate.candidate_id for candidate in pool.candidates if candidate.text == "今日は東京に行く"): 0.40,
        next(candidate.candidate_id for candidate in pool.candidates if candidate.text == "今日は東京へ行く"): 0.35,
        next(candidate.candidate_id for candidate in pool.candidates if candidate.text == "今日は京都に行く"): 0.25,
    }
