...
def _softmax(values: Sequence[float], temperature: float = 1.0) -> list[float]:
    ...
    maximum = max(values)
    exponentials = [math.exp(max(-80.0, min(80.0, (value - maximum) / temperature))) for value in values]
...
class FeatureNormalizer:
    ...
    def fit(cls, schema: FeatureSchema, groups: Sequence[RankingGroup]) -> Self:
        ...
        if any(candidate.features.schema_digest != schema.digest for group in groups for candidate in group.candidates):
...
def _listwise_metric(groups: Sequence[RankingGroup], model: ConstrainedLinearReranker) -> float:
    losses: list[float] = []
    for group in groups:
        target = _softmax([-candidate.target_loss for candidate in group.candidates], temperature=0.15)
        predicted = _softmax([model.score(candidate.features) for candidate in group.candidates])
        losses.append(-sum(p * math.log(max(1e-12, q)) for p, q in zip(target, predicted, strict=True)))
    ...
    for epoch in range(epochs):
        ...
        for group in order:
            if objective in {"pairwise", "hybrid"}:
                ...
                for index, left in enumerate(group.candidates):
                    for right in group.candidates[index + 1 :]:
                        combined_left = (1 - critical_loss_weight) * left.target_loss + critical_loss_weight * left.critical_loss
                        combined_right = (1 - critical_loss_weight) * right.target_loss + critical_loss_weight * right.critical_loss
                        if math.isclose(combined_left, combined_right, abs_tol=1e-12):
                            continue
                        pairs.append((left, right) if combined_left < combined_right else (right, left))
                rng.shuffle(pairs)
                for preferred, other in pairs:
                    ...
                    margin = sum(weights[name] * (preferred_x[name] - other_x[name]) for name in schema.names)
            ...
            if objective in {"listwise", "hybrid"}:
                ...
                logits = [
                    ...
                    + sum(
                        weights[name]
                        * transformed[(group.group_id, candidate.candidate_id)][name]
                        for name in schema.names
                    )
...
def features_from_candidate_evidence(
    ...
    *,
    ...
    rank_fraction = (
        ...
        else (hypothesis_count - rank) / (hypothesis_count - 1)
    )
    streams = (
        candidate.acoustic,
        candidate.mora,
    ...
    values = {
        ...
        "teacher_preference": float(
            teacher_preference
            if teacher_preference is not None
            else candidate.teacher or 0.0
        ),
