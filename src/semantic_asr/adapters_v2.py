...
class PathDecodeRequest:
    audio_path: str
    variants: tuple[DecodeVariant, ...] = (
        DecodeVariant("beam-5", beam_size=5, hypotheses=5),
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not self.variants or len({variant.variant_id for variant in self.variants}) != len(self.variants):
    ...
    def decode(self, request: PathDecodeRequest) -> CandidatePool:
        ...
        for variant in request.variants:
            ...
            for raw_rank, (raw_tokens, normalized_score) in enumerate(
                ...
                cumulative = float(normalized_score) * (
                    len(token_ids) ** variant.length_penalty
                )
