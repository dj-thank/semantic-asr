...
class DatasetManifest:
    ...
    def digest(self) -> str:
        payload = {
            ...
            "records": [asdict(record) for record in sorted(self.records, key=lambda row: row.sample_id)],
        }
    ...
    def leakage_findings(self, *, reference_near_duplicate: bool = True) -> tuple[LeakageFinding, ...]:
    ...
    def assert_leakage_free(self, *, reference_near_duplicate: bool = True) -> None:
        findings = self.leakage_findings(
...
def paired_bootstrap_comparison(
    ...
    *,
    ...
    shared = sorted(
        set(by_system[baseline_system]) & set(by_system[candidate_system])
    )
    if not shared:
    ...
    *,
    ...
    for result in results:
        ...
        for slice_name, slicer in slicers.items():
            ...
            grouped.setdefault(
                (result.system_id, slice_name, slice_value), []
...
class QualityCostPoint:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            for value in (self.quality_loss, self.cost)
        ):
...
def quality_cost_frontier(
    ...
    frontier = [
        ...
        if not any(
            ...
            and (
                other.quality_loss < candidate.quality_loss
    ...
    return tuple(
        sorted(frontier, key=lambda row: (row.cost, row.quality_loss, row.system_id))
    )
...
def evaluate_claim_gate(
    ...
    *,
    ...
    reasons: list[str] = []
    improvement = (
        comparison.upper_delta < 0
        if comparison.lower_is_better
        else comparison.lower_delta > 0
    )
    if not improvement:
