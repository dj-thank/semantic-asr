"""Joint document-level decoding over window-local Semantic ASR lattices.

The v0.3 long-form second pass scores one window at a time against frozen neighboring context. This
module keeps several policy-eligible paths per window and searches their Cartesian product with a
bounded document beam. Window overlap, local acoustic support, complete-document language evidence,
and the immutable first-pass path are therefore considered in one decision.

The default v0.2 transcription path is not modified. All entry points here are explicit research
surfaces and fail closed to the first-pass result unless the caller asks otherwise.
"""
...
class DocumentProposalProvider(Protocol):
    """Acquire source-audio-bound proposals for one already-built window lattice."""
        ...
        return sha256_json(
            {
                **asdict(self),
                ...
                "decoder": "hierarchical-document-beam-v1",
            }
        )
...
class DocumentPathCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(option.segment_index for option in self.options) != expected:
        ...
        for option, emitted, receipt in zip(
            ...
            if hashlib.sha256(emitted.encode("utf-8")).hexdigest() != receipt.emitted_text_sha256:
        ...
        return sha256_json(
            {
                ...
                "emittedTextSha256": [
                    ...
                    for text in self.emitted_texts
                ],
...
class DocumentDeliberationDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if self.applied and self.status == "provisional":
            # Applying provisional output is permitted only through the explicit config. The reason
            # remains recorded so downstream consumers do not mistake it for accepted evidence.
            if "explicitly-applied-provisional" not in self.reasons:
...
class DocumentObservedTranscript:
    """One emitted window slice selected by a document-level path."""
    ...
    def create(
        ...
        *,
        ...
        selected_candidate_id = f"document-path-{option.path.digest[:20]}"
        arc_receipts = tuple(
            DocumentArcReceipt(
                arc_id=arc.arc_id,
                span_id=arc.span_id,
...
class _BeamState:
    ...
    def mean_audio_support(self) -> float:
        if self.audio_weight <= 0.0:
            return -1.0
        return self.weighted_audio_sum / self.audio_weight
...
def resolve_window_overlap(
    ...
    *,
    ...
    """Return the safe emitted suffix of one window and an immutable overlap receipt."""

    if not right_text:
        raise ValueError("right window text must not be empty")
    right_digest = _text_sha256(right_text)
    if left_text is None:
        emitted = right_text
        return emitted, OverlapReceipt(
            ...
            right_text_sha256=right_digest,
    ...
    if overlap_ms <= 0:
        ...
        return emitted, OverlapReceipt(
            ...
            similarity=0.0,
            utility=0.0,
            left_text_sha256=_text_sha256(left_text),
            right_text_sha256=right_digest,
            emitted_text_sha256=_text_sha256(emitted),
            policy_digest=policy.digest,
        )

    exact = _longest_suffix_prefix(
        left_text,
        right_text,
    ...
    if exact >= policy.minimum_exact_characters:
        ...
        return emitted, OverlapReceipt(
            ...
            method="exact-suffix-prefix",
            right_trim_characters=exact,
            matched_characters=exact,
            normalized_matched_characters=exact,
            similarity=1.0,
            utility=utility,
            left_text_sha256=_text_sha256(left_text),
            right_text_sha256=right_digest,
    ...
    normalized_left, _left_map = _normalization_map(left_text)
    ...
    if normalized_match >= policy.minimum_normalized_characters and right_map:
        ...
        return emitted, OverlapReceipt(
            ...
            right_window_index=right_window_index,
            overlap_ms=overlap_ms,
            method="normalized-suffix-prefix",
            right_trim_characters=trim,
            matched_characters=0,
            normalized_matched_characters=normalized_match,
            similarity=1.0,
            utility=utility,
            left_text_sha256=_text_sha256(left_text),
            right_text_sha256=right_digest,
...
def _fallback_build(
    ...
    *,
    ...
    return build_semantic_deliberation_lattice(
        ...
        config=SemanticDeliberationConfig(include_consensus_utilities=True),
    )
...
def _window_options(
    ...
    *,
    ...
    proposal_provider: DocumentProposalProvider | SpanProposalProvider | None,
...
def _window_audio_weight(option: WindowPathOption) -> float:
    if option.path.mean_audio_support < -0.999999:
        return 0.0
    return max(1.0, option.window.end_ms - option.window.start_ms)
...
def _changed_limit(config: DocumentDeliberationConfig, window_count: int) -> int:
    ...
    if config.maximum_changed_ratio > 0.0:
    ...
    for index, window_options in enumerate(options_by_window):
        ...
        for state in states:
            ...
            for option in window_options:
                ...
                audio_weight = _window_audio_weight(option)
                expanded.append(
                    _BeamState(
                        ...
                        overlap_score=(
                            state.overlap_score + config.overlap_weight * receipt.utility
                        ),
                        weighted_audio_sum=(
                            state.weighted_audio_sum
                            + audio_weight * option.path.mean_audio_support
                        ),
                        audio_weight=state.audio_weight + audio_weight,
                    )
                )
        if not expanded:
...
def _retained_document_candidate(
    ...
    *,
    ...
    retained_rows = []
    for rows in options_by_window:
        retained = next(
            row for row in rows if row.path.digest == row.retained_path_digest
        )
        retained_rows.append((retained,))
    return _expand_document_beam(retained_rows, config=config)[0]
...
def _span_resolutions(option: WindowPathOption) -> tuple[SpanResolution, ...]:
    output = []
    for span, selected in zip(
        option.build.lattice.spans,
        option.path.arcs,
        strict=True,
    ):
        retained = span.retained_arc
        if selected.arc_id == retained.arc_id:
            mode = "retained-first-pass"
        elif (
            selected.pronunciation_key is not None
            and selected.pronunciation_key == retained.pronunciation_key
        ):
            mode = "context-resolved-orthography"
        elif selected.origin in GENERATED_ORIGINS:
            mode = "acoustically-verified-proposal"
        else:
            mode = "acoustic-context-consensus"
        output.append(
            SpanResolution(
                span_id=span.span_id,
                retained_arc_id=retained.arc_id,
                selected_arc_id=selected.arc_id,
                mode=mode,
...
def _local_decision(
    option: WindowPathOption,
    alternatives: Sequence[WindowPathOption],
    *,
    document_decision: DocumentDeliberationDecision,
    local_policy: DeliberationPolicy,
    context: DocumentContext,
    ...
    retained = next(
        row.path for row in alternatives if row.path.digest == row.retained_path_digest
    )
    return GlobalDeliberationDecision(
        selected=option.path,
        retained=retained,
        alternatives=tuple(row.path for row in alternatives),
        status=document_decision.status,
        margin=document_decision.margin,
        resolutions=_span_resolutions(option),
        reasons=(
            "selected-by-joint-document-beam",
            *document_decision.reasons,
        ),
        lattice_digest=option.build.lattice.digest,
        policy_digest=local_policy.digest,
        context_digest=context.digest,
        scorer_source=document_decision.selected.scorer_source,
        scorer_profile_digest=document_decision.selected.scorer_profile_digest,
    )
...
def _build_result(
    ...
    *,
    ...
    selected = decision.selected if decision.applied else decision.retained
    segments: list[DocumentDeliberatedSegment] = []
    for index, (option, emitted, receipt) in enumerate(
        zip(
            selected.options,
            selected.emitted_texts,
            selected.overlap_receipts,
            strict=True,
        )
    ):
            ...
            DocumentDeliberatedSegment(
                ...
                diagnostics={
                    **dict(first_segment.diagnostics),
                    ...
                    "windowOptionDigest": option.digest,
                    "localDecisionDigest": local.digest,
    ...
    diagnostics = {
        **dict(first_pass.diagnostics),
        "provisionalWindowCount": (
            len(segments) if decision.status == "provisional" else 0
        ),
...
def apply_joint_document_deliberation(
    ...
    *,
    ...
    proposal_provider: DocumentProposalProvider | SpanProposalProvider | None = None,
    declared_context: DocumentContext | None = None,
    audio_path: str | Path | None = None,
) -> LongformResult | DocumentDeliberatedResult:
    """Jointly select window paths and overlap emissions for one frozen first-pass document."""
    ...
    try:
        ...
        base_candidates = _expand_document_beam(options_by_window, config=config)
        retained = _retained_document_candidate(options_by_window, config=config)
        guarded = tuple(
            row
            for row in base_candidates
            if retained.mean_audio_support - row.mean_audio_support
            <= config.maximum_document_audio_regression
        )
        if retained.digest not in {row.digest for row in guarded}:
        ...
        retained_scored = next(
            (row for row in scored if row.digest == retained.digest),
            retained,
        )
        ...
        if selected.digest == retained_scored.digest:
            reasons.append("retained-first-pass-document")
        apply_selected = selected.digest != retained_scored.digest and (
            status == "accepted" or config.apply_provisional
        )
        if status == "provisional" and apply_selected:
            reasons.append("explicitly-applied-provisional")
        elif status == "provisional" and selected.digest != retained_scored.digest:
...
class JointDocumentSemanticASRTranscriber:
    """Composition wrapper that performs first pass once and then joint document decoding."""
