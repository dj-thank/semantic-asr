...
class SequenceScorer(Protocol):
    ...
    def score(
        self, candidates: list[TextCandidate], *, context: str = ""
...
class CausalScoringConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not math.isfinite(self.length_normalization_alpha)
            or self.length_normalization_alpha <= 0
        ):
...
class TransformersCausalSequenceScorer:
    ...
    def _encoded_candidate(self, context: str, candidate: TextCandidate) -> tuple[list[int], int]:
        ...
        candidate_ids = self._ids(
            candidate.text, add_special_tokens=not context_ids and self.config.add_bos
        )
        if (
            context_ids
            and candidate_ids
        ...
        if len(candidate_ids) > self.config.maximum_candidate_tokens:
            raise ValueError(f"candidate {candidate.candidate_id} exceeds maximum_candidate_tokens")
    ...
    def score(
        ...
        *,
        ...
        for batch_start in range(0, len(candidates), self.config.batch_size):
            ...
            token_log_probs = (
                logits.log_softmax(dim=-1).gather(-1, labels.unsqueeze(-1)).squeeze(-1)
            )
