...
class KoemoMeetingEvidence:
    ...
    def __post_init__(self) -> None:
        if not self.meeting_id or not self.semantic_asr_revision or not self.configuration_digest:
            raise ValueError(
                "meeting ID, Semantic ASR revision and configuration digest are required"
            )
    ...
    def authoritative_events(self) -> tuple[KoemoTranscriptEvent, ...]:
        return tuple(event for event in self.events if event.authority == "final-observed")

    def normalized_events(self) -> tuple[KoemoTranscriptEvent, ...]:
        return tuple(event for event in self.events if event.authority == "normalized-derivative")
