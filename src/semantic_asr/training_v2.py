...
class SparseEvidenceReranker(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        acoustic_features = candidate_features[
            :, :, list(self.acoustic_feature_indices)
        ]
...
class MultiObjectiveRankingLoss(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        expected_critical = (
            probabilities * critical_losses.masked_fill(~mask, 0)
        ).sum(dim=1).mean()
        ...
        pairwise = (
            torch.stack(pairwise_terms).mean()
            if pairwise_terms
            else logits.sum() * 0.0
        )
...
class AcousticTextVerifier(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if torch.any(~audio_mask.bool().any(dim=1)) or torch.any(
            ~text_mask.bool().any(dim=2)
        ):
...
def verifier_loss(
    ...
    *,
    ...
    valid = torch.ones_like(labels, dtype=torch.bool) if candidate_mask is None else candidate_mask.bool()
