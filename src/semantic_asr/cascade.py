...
DecisionStatus = Literal["unresolved", "accepted", "provisional", "rejected"]
StageKind = Literal[
    "generator",
    "scorer",
    "reranker",
...
class StageEstimate:
    ...
    def __post_init__(self) -> None:
        ...
        if self.memory_mb is not None and (
            self.memory_mb < 0 or not math.isfinite(self.memory_mb)
        ):
        ...
        *,
        ...
        for stage in self.stages:
            ...
            if execution.state.traces != state.traces:
                raise ValueError(
                    "stages must not mutate traces; the controller owns the audit log"
                )
